...
## Namenseingabe konsolidiert, Video-Limit erhöht

**Namenseingabe:** Der separate `/name`-Screen ist entfernt – die Eingabe passiert jetzt ausschließlich auf der Landing Page (kein doppelter Schritt mehr). Dabei wurde auch ein Bug behoben: die lokale Eingabe-State-Variable hieß `setName`, identisch zur globalen `setName`-Funktion aus `usePlayer()`, die als Prop reinkam – die Prop wurde dadurch unsichtbar überschrieben und der eingegebene Name landete nie dauerhaft in `localStorage`. Die Prop heißt jetzt `setPlayerName` zur klaren Unterscheidung. Außerdem neu: Zeichen-Counter (x/16) und ein Hinweistext bei zu kurzer Eingabe.

**Video-Upload-Limit:** Videos werden weiterhin als Base64 direkt im Firestore-Dokument gespeichert (keine Firebase Storage, um den Blaze-Plan zu vermeiden – siehe Abschnitt oben). Das bisherige Budget (~700 KB, 12s) war aber unnötig konservativ kalkuliert: es ging von ~300 KB Overhead für den Rest des Dokuments aus, real sind es nur ~2 KB (Spielerliste, Settings, Votes, Timestamps). Neu durchgerechnet: `(1 MB Firestore-Limit − 8 KB Sicherheitspuffer) × ¾ (Base64-Dekodierung) × 0.92 (Schreibmarge) ≈ 701 KB Rohvideo-Budget`. Aufnahmedauer von 12s auf 20s erhöht, Auflösung von 480×854 auf 540×960 angehoben – beides passt bei 250 kbps Bitrate sicher ins neue Budget (≈610 KB für ein volles 20s-Video).
